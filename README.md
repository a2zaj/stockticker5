...
> Note: Minecraft 26.2 itself requires Java 25 to run (this became the requirement
> starting with the 26.1 "Tiny Takeover" drop), so the compile target and your
> server's runtime JVM are already aligned — no mismatch to worry about here,
> unlike the older 1.21.x days.
...
> to run BuildTools first (https://www.spigotmc.org/wiki/buildtools/) with
> `--rev 26.2` to install `spigot-api-26.2-R0.1-SNAPSHOT` into your local Maven
> repo, OR just switch the pom.xml dependency to a public mirror if you use one.
...
The tape cycles through `TICKER ▲/▼ $price ±change%` for each stock in your
list, separated by `|`, looping continuously right-to-left. Each stock is
individually colored green (▲) or red (▼) based on its own direction — the
separators stay neutral gray. Scroll speed is set server-wide via
`tape-scroll-ticks` in `config.yml` (default `4` — every 4 ticks, about 5
times a second).
...
- Tape text is colored per-stock (green/red based on that stock's own
  direction) rather than embedding color codes directly in the scrolled
  string. Color is tracked in a parallel array alongside the plain text and
  re-applied fresh to whatever's visible each tick — this avoids a subtler bug
  where slicing a fixed-width window through pre-colored text could land
  exactly between a § character and its color code, flashing a stray
  character at the window boundary.
...
If a ticker's data stops updating (e.g. an API key goes bad, or Yahoo's
unofficial futures endpoint starts rate-limiting/blocking the server —
this happens to it periodically since it's not a real public API), signs,
big displays, and ticker tapes will now show **"no data / since HH:mm"**
instead of silently freezing on the last price forever. Controlled by
`stale-after-seconds` in `config.yml` (default `120`) — how long a quote can
go without refreshing before it's flagged as stale rather than shown as live.
